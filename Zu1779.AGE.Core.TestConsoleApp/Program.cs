namespace Zu1779.AGE.Core.TestServerConsoleApp
{
    using System;
    ...
    using Zu1779.AGE.Core.MainEngine;
    //using Zu1779.AGE.Wcf;

    class Program
    {
        private static readonly log.ILogger logger = log.LogManager.GetCurrentClassLogger();
        ...
        public void Execute()
        {
            using (engineManager = new EngineManager())
            {
                ...
                while (input != "exit")
                {
                    ...
                    {
                        logger.Error($"Command {input} failed: {ex.Message}");
                    }
                }
        ...
        {
            logger.Info($"Starting WCF Interface");
            //var ageWcfService = new AgeWcfService(engineManager);
            //serviceHost = new ServiceHost(ageWcfService);
            //serviceHost.Open();
            logger.Info($"Started WCF Interface");
        }

        private void StopWcfInterface()
        {
            logger.Warn($"Stopping WCF Interface");
            //if (serviceHost != null)
            //{
            //    serviceHost.Close();
            //    serviceHost = null;
            //}
            logger.Warn($"Stopped WCF Interface");
        }
        #endregion
    }
}
